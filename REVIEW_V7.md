...
## 六、部署验证（宿主机 124.222.198.26 实测）

### 部署状态

| 项目 | 状态 |
|------|------|
| 容器 | `item-monitor` Up 25+ hours (healthy) |
| 运行模式 | **JSON 文件模式**（`USE_SQLITE` 未设置） |
| 镜像构建时间 | 39 小时前 |
| 代码同步 | ❌ **容器代码落后于仓库** |

### 发现：部署版本落后

通过对比容器内代码与仓库代码，发现以下**关键差异**：

| 问题 | 仓库代码 (最新) | 容器代码 (部署版本) | 影响 |
|------|----------------|-------------------|------|
| `save_certs` SQLite 批量保存 | ✅ 单事务批量 `INSERT OR REPLACE` | ❌ 逐条调用 `db_save_cert(c)` | V6 P0-8 未部署 |
| `_check_api_csrf` GET 豁免 | ✅ `if request.method == "GET": return True` | ❌ 没有 GET 豁免 | V6 P1-8 未部署 |
| CSRF token 轮换条件 | ✅ 仅 POST/PUT/DELETE/DELETE 轮换 | ❌ 所有请求都轮换 | 可能导致 GET API 拿不到 token |
| CSP header | ✅ 正确注入 nonce | ❌ **完全注释掉** | 手机浏览器兼容性问题 |
| CSP nonce 线程安全 | ✅ 使用 `g.csp_nonce` | ❌ 使用全局变量 `_csp_nonce` | 多线程下 nonce 可能串号 |
| `api_list_certs` CSRF | ✅ `session.get("_csrf_token", "")` | ❌ 可能 KeyError | V6 P0-7 未部署 |
...
1. **立即部署 V6 修复** — 容器内的代码缺少 V6 所有修复（P0-7, P0-8, P1-8, P1-9, P2-4）
2. **解决 CSP 手机兼容问题** — 不要注释掉 CSP，而是修复前端内联脚本使其兼容 nonce
3. **启用 SQLite 模式** — `monitor.db` 已存在但未使用，建议设置 `USE_SQLITE=1`
4. **修复全局 CSP nonce** — 部署版本使用全局变量 `_csp_nonce`，多线程下不安全

---

## 七、综合评分（含部署验证）

| 维度 | V5 | V6 | V7 (代码) | V7+ (含部署) |
|------|----|----|-----------|-------------|
| 架构改进 | 9 | 9.5 | **9.5** | 9.0 |
| 安全性 | 8.5 | 9 | **9** | 7.5 |
| 代码质量 | 9 | 8.5 | **8.5** | 8.0 |
...
**部署同步评分低是因为：代码仓库已经修复了 V6 所有问题，但生产环境还在用旧代码。**
