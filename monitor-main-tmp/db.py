            ...
            if existing:
                ...
                conn.execute("""UPDATE certs SET customer=?, cert_type=?, domain=?, expire_date=?,
                    ...
                    (cert_data["customer"], cert_data.get("cert_type", ""), cert_data.get("domain", ""),
                     ...
                     json.dumps(cert_data.get("responsible_users") if cert_data.get("responsible_users") is not None else [], ensure_ascii=False),
                     now, cert_id))
            else:
                # 不存在则 INSERT
        ...
        if existing:
            ...
            conn.execute("""UPDATE users SET name=?, password=?, dingtalk_id=?,
                ...
                (user_data.get("name", user_data["username"]),
                 ...
                 int(user_data.get("force_change_password", 0)),
                 user_data["username"]))
        else:
            # 不存在则 INSERT
