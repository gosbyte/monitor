...
def _build_badge(status: str, days_left: float, expire_str: str) -> str:
    ...
    safe_expire = escape(expire_str)
    badges = {
        "disabled": f'<span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-600" title="到期日期：{safe_expire}"><i data-lucide="bell-off" class="w-3 h-3"></i> 已禁用</span>',
        "expired": f'<span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800" title="到期日期：{safe_expire}"><i data-lucide="x" class="w-3 h-3"></i> 已过期 {days}天</span>',
        "expiring": f'<span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-orange-100 text-orange-800" title="到期日期：{safe_expire}"><i data-lucide="alert-triangle" class="w-3 h-3"></i> {days}天后</span>',
        "normal": f'<span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-blue-50 text-blue-700" title="到期日期：{safe_expire}"><i data-lucide="check-circle" class="w-3 h-3"></i> {days}天</span>',
    }
    return badges.get(status, f'<span>{safe_expire}</span>')
...
def register_cert_routes(app: Flask) -> None:
    ...
    def add_cert() -> _FlaskResponse:
        ...
        # [FIX] P0-3: 添加到期项
        from db import db_calc_stats,  db_save_cert, get_db
        ...
        write_log(session.get("username", "?"), "添加记录", customer, "到期项", get_client_ip(request))
    ...
    def edit_cert(cert_id: int) -> _FlaskResponse:
        ...
        if request.method == "POST":
            ...
            save_certs(certs)
            write_log(session.get("username", "?"), f"编辑记录 #{cert_id}", cert.get("customer", ""), "到期项", get_client_ip(request))
            if is_ajax:
    ...
    def delete_cert(cert_id: int) -> _FlaskResponse:
        ...
        save_certs(certs)
        write_log(session.get("username", "?"), f"删除记录 #{cert_id}", cert_name, "到期项", get_client_ip(request))
        if is_ajax:
    ...
    def api_delete_cert(cert_id: int) -> Any:
        ...
        certs = [c for c in certs if c["id"] != cert_id]
        save_certs(certs)
        write_log(current_username, f"删除记录 #{cert_id}", cert_name, "到期项", get_client_ip(request))
        return jsonify({"ok": True, "message": "删除成功", "csrf_token": session.get("_csrf_token", "")})
    ...
    def import_certs() -> Any:
        ...
        try:
            ...
            save_certs(certs)
            write_log(session.get("username", "?"), "批量导入", f"共 {imported} 条记录", "到期项", get_client_ip(request))
    ...
    def api_import_excel() -> Any:
        ...
        for i, r in enumerate(rows):
            ...
            imported += 1
        save_certs(certs)
        write_log(session.get("username", "?"), "Excel导入", f"成功导入 {imported} 条，失败 {len(errors)} 条", "到期项", get_client_ip(request))
    ...
    def backup_data() -> Response:
        from db import db_calc_stats,  db_load_certs, db_load_logs, db_load_push_history, db_load_config, get_db
        certs = db_load_certs()
        logs = db_load_logs()
    ...
    def restore_data() -> Any:
        ...
        try:
            from db import db_calc_stats, db_batch_delete_cert_ids
        ...
        write_log(session["username"], "恢复数据", "系统", f"从备份恢复（{file.filename}）", get_client_ip(request))
