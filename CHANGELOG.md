...
### 🎨 Frontend UI/UX (2026-07-04)

#### 整体架构
- **统一服务器版本为主力分支** — 保留服务器版本的全部优化，丢弃小黑分支的回退改动
- **三点菜单移到 body 下** — 使用 `insertAdjacentHTML` 动态挂载到 body，避免被 `.relative.group` 裁剪，配合 `closeRowMenu()` 防止点击关闭

#### 交互体验
- **搜索建议下拉框** — 输入时自动从表格数据匹配客户名/证书类型，支持 ↑↓ 导航、Enter 确认、Escape 关闭
- **键盘快捷键** — `Ctrl+K` 聚焦搜索框、`Ctrl+N` 新建记录、`Escape` 关闭弹窗
- **标记已处理确认弹窗** — 操作前弹出确认对话框，显示客户名称和操作后果
- **复制记录确认弹窗** — 防止误操作，显示客户名称
- **按钮防重复点击** — 异步操作期间按钮变灰禁用，`.finally()` 恢复
- **状态切换视觉反馈** — 按钮即时变灰 → API 返回 → 恢复并更新图标

#### 视觉与动效
- **状态徽章增强 Tooltip** — 悬停显示到期日期 + 剩余天数 + 操作提示
- **行 Hover 微移** — `translateX(1px)` + 背景色变化 + 左侧蓝色指示条
- **操作按钮微交互** — Hover `scale(1.1)`、Active `scale(0.95)`
- **统计卡片悬浮效果** — Hover `translateY(-2px)` + 阴影加深
- **Toast 进度条动画** — 3 秒倒计时进度条，slide-in/slide-out 动画
- **行滑入动画** — 新行从左向右滑入（`row-animate` + `rowFadeIn`）
- **空状态改进** — `fadeIn` 淡入动画，更友好的引导文案

#### 功能增强
- **创建人列** — 新增 `created_by` 列，可点击排序，`hidden lg:table-cell` 响应式隐藏
- **页面标题栏** — 标题 + 副标题 + 最后更新时间（实时更新）
- **重置筛选按钮** — 一键清除所有筛选条件（状态、类型、日期范围、搜索词）
- **排序图标** — 排序列表头显示 ▲/▼ 指示当前排序方向和状态
- **批量操作工具栏** — 多选复选框 + 批量标记已处理/启用提醒/删除
- **响应式列隐藏** — 备注列 `md:` 断点、创建人列 `lg:` 断点

#### 性能优化
- **filterTable DOM 查询缓存** — 避免重复 querySelectorAll
- **搜索建议缓存** — `_buildSearchCache()` 首次构建后缓存，表格数据变化时重建

#### 代码变更
- `templates/index.html` — 全面 UI/UX 优化（+480/-99 行）
- 保留服务器版本全部已有优化，合并小黑分支有价值的改进
